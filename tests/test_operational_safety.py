...
class OpportunityDeskSafety(unittest.TestCase):
    ...
    def test_persistence_requires_absolute_usable_root(self):
        with patch.object(dealdesk, "_PATH", "relative/dealdesk.json"):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(dealdesk, "_PATH", str(missing_root)):
                self.assertEqual(dealdesk.persistence_state(), "UNAVAILABLE")
                self.assertFalse(dealdesk.persistence_configured())

            with patch.object(dealdesk, "_PATH", directory):
                self.assertEqual(dealdesk.persistence_state(), "UNAVAILABLE")
                self.assertFalse(dealdesk.persistence_configured())
