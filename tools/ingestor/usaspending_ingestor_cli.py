"""CLI entry for the USAspending lane (CI-only, never the Space).

Usage: python -m tools.ingestor.usaspending_ingestor_cli --zip <path> --out <dir>
"""
...
from tools.ingestor.usaspending_ingestor import run_usaspending


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--zip", required=True, help="Path to downloaded USAspending award archive ZIP")
    ap.add_argument("--out", required=True, help="Output directory for the snapshot")
    args = ap.parse_args()

    payload = Path(args.zip).read_bytes()
    result = run_usaspending(payload)
