"""CLI entry for the FMCSA MOTUS ingestor lane (CI-only, never the Space).

Usage: python -m tools.ingestor.motus_ingestor_cli --csv <path> --out <dir>
"""
...
from tools.ingestor.motus_ingestor import run_motus


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--csv", required=True, help="Path to downloaded MOTUS Carrier CSV")
    ap.add_argument("--out", required=True, help="Output directory for the snapshot")
    args = ap.parse_args()

    payload = Path(args.csv).read_bytes()
    result = run_motus(payload)
