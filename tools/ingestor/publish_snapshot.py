...
def _dataset_card(snapshot: dict[str, object], prefix: str) -> bytes:
    ...
    return (
        ...
        "Facility names are official EPA free text. The ingestor rejects rows whose "
        "name reads as a private individual or a private residence (NAICS 814110, "
        "residential permit designations, a house number with a street type, or a "
        "personal name without an organization word) and counts them as "
        "`PERSON_OR_RESIDENCE_NAME` in `snapshot.json`. This name screen is a "
        "heuristic; snapshots published before it was added did not apply it.\n\n"
